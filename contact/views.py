...
def send_contact_email(contact):
    ...
    # Email to customer
    customer_subject = render_to_string("contact/confirmation_emails/confirmation_email_subject.txt")
    customer_html_body = render_to_string("contact/confirmation_emails/confirmation_email_body.html", {"contact": contact, "company_email": from_email})
    customer_text_body = strip_tags(customer_html_body)

    msg_to_customer = EmailMultiAlternatives(customer_subject, customer_text_body, from_email, [contact.email])
    ...
    owner_html_body = render_to_string("contact/confirmation_emails/owner_notification_email.html", {"contact": contact})
    owner_text_body = strip_tags(owner_html_body)

    msg_to_owner = EmailMultiAlternatives(owner_subject, owner_text_body, from_email, [from_email])
